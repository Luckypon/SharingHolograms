...
public static class SimplePool
{
    ...
    class Pool
    {
        ...
        public GameObject Spawn(Vector3 localPos, Quaternion localRot, Transform parent)
        {
            GameObject obj;
            if (inactive.Count == 0)
            {
                // We don't have an object in our pool, so we
                // instantiate a whole new object.
                obj = (GameObject)GameObject.Instantiate(prefab, localPos, localRot);
                obj.name = prefab.name + " (" + (nextId++) + ")";
            ...
            else
            {
                ...
                if (obj == null)
                {
                    ...
                    return Spawn(localPos, localRot);
                }
            }
    ...
    static public GameObject Spawn(GameObject prefab, Vector3 localPos, Quaternion localRot, Transform parent)
    {
        Init(prefab);

        return pools[prefab].Spawn(localPos, localRot, parent);
    }
